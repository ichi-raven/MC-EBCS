...
BuiltScene::BuiltScene(BasicScene* pScene) : pParsedScene(pScene) {
    auto &parsedScene = *pParsedScene;
    Allocator alloc;
    ThreadLocal<Allocator> threadAllocators([]() { return Allocator(); });

    // Create media first (so have them for the camera...)
    media = parsedScene.CreateMedia();

    // Textures
    LOG_VERBOSE("Starting textures");
    textures = parsedScene.CreateTextures();
    LOG_VERBOSE("Finished textures");

    // Lights
    std::map<int, pstd::vector<Light> *> shapeIndexToAreaLights;
    lights = parsedScene.CreateLights(textures, &shapeIndexToAreaLights);
    ...
    accel = parsedScene.CreateAggregate(textures, shapeIndexToAreaLights, media,
                                        namedMaterials, materials);

    camera = parsedScene.GetCamera();
    film = camera.GetFilm();
    sampler = parsedScene.GetSampler();
 }

void ParseFilesIndependent(pstd::vector<ParserTarget *> targets,
                           pstd::span<const std::string> filenames,
                           std::vector<BuiltScene> &builtScenes_out)
{
    auto tokError = [](const char *msg, const FileLoc *loc) {
        ErrorExit(loc, "%s", msg);
    };

    assert(targets.size() == filenames.size() || !"invalid target size!");
    ...
    if (filenames.empty()) {
        ...
        std::unique_ptr<Tokenizer> t = Tokenizer::CreateFromFile("-", tokError);
        if (t) {
            parse(targets.front(), std::move(t));
        }
        ...
        //for (std::size_t i = 0; i < 1; ++i) {
            ...
            std::unique_ptr<Tokenizer> t = Tokenizer::CreateFromFile(fn, tokError);
            if (t)
                parse(targets[i], std::move(t));

            targets[i]->EndOfFiles();
            
            builtScenes_out.emplace_back(
                dynamic_cast<BasicSceneBuilder *>(targets[i])->getpScene());

            std::cout << "loaded file name : " << fn << "\n";
        }
    }
}
