...
class PixelSensor {
  public:
    ...
    PixelSensor(Spectrum r, Spectrum g, Spectrum b, const RGBColorSpace *outputColorSpace,
        ...
        : r_bar(r, alloc),
          g_bar(g, alloc),
          b_bar(b, alloc),
          r_bar_sq(r, 360 * 360, 830 * 830, alloc),
          g_bar_sq(g, 360 * 360, 830 * 830, alloc),
          b_bar_sq(b, 360 * 360, 830 * 830, alloc),
  ...
  private:
    ...
    DenselySampledSpectrum r_bar_sq, g_bar_sq, b_bar_sq;
    Float imagingRatio;
...
class FilmBase {
  public:
    ...
    PBRT_CPU_GPU
    SampledWavelengths SampleWavelengthsSquare(Float u) const {
        return SampledWavelengths::SampleVisibleSquare(u);
    }
...
class RGBFilm : public FilmBase {
  public:
    ...
    const RGBColorSpace *getColorSpace() const { return colorSpace; }
...
class GBufferFilm : public FilmBase {
  public:
    ...
    PBRT_CPU_GPU
    void AddSampleLogGray(Point2i pFilm, SampledSpectrum L,
                         const SampledWavelengths &lambda,
                         const VisibleSurface *visibleSurface, Float weight, Float threshold);

    PBRT_CPU_GPU
    void AddSampleSquare(Point2i pFilm, SampledSpectrum L,
                         const SampledWavelengths &lambda,
                         const VisibleSurface *visibleSurface, Float weight,
                         Float threshold);
    ...
    RGB GetPixelRGB(Point2i p, Float splatScale = 1) const {
        ...
        PBRT_CPU_GPU
    RGB GetPixelNativeRGB(Point2i p, Float splatScale = 1) const {
        const Pixel &pixel = pixels[p];
        RGB rgb(pixel.rgbSum[0], pixel.rgbSum[1], pixel.rgbSum[2]);
    ...
    // HACK:
    AnimatedTransform getAnimatedTransform() { return outputFromRender; }
    const RGBColorSpace *getColorSpace() { return colorSpace; }
    bool getApplyInverse() { return applyInverse; }
...
PBRT_CPU_GPU
inline SampledWavelengths Film::SampleWavelengthsSquare(Float u) const {
    auto sample = [&](auto ptr) { return ptr->SampleWavelengthsSquare(u); };
    return Dispatch(sample);
}
