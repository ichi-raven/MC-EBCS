...
constexpr Float Lambda_min_sq = Lambda_min * Lambda_min,
                Lambda_max_sq = Lambda_max * Lambda_max;
...
class SampledSpectrum {
  public:
    ...
    SampledSpectrum Sqrt() const {
        SampledSpectrum ret;
        for (int i = 0; i < NSpectrumSamples; ++i)
            ret.values[i] = pstd::sqrt(values[i]);
        return ret;
    }

    template <typename Func>
    PBRT_CPU_GPU SampledSpectrum each(Func func) const {
        SampledSpectrum ret;
        for (int i = 0; i < NSpectrumSamples; ++i)
            ret.values[i] = func(values[i]);
        return ret;
    }
...
class SampledWavelengths {
  public:
    ...
    PBRT_CPU_GPU
    static SampledWavelengths SampleConstant(Float R, Float G, Float B) {
        SampledWavelengths swl;
        swl.lambda[0] = R;
        swl.lambda[1] = G;
        swl.lambda[2] = B;
        ...
        swl.pdf[NSpectrumSamples - 1] = 0.0;
        return swl;
    }

    PBRT_CPU_GPU
    static SampledWavelengths squarePDF(SampledWavelengths swl) {
        for (int i = 0; i < NSpectrumSamples - 1; ++i)
            swl.pdf[i] *= swl.pdf[i];
        return swl;
    }

    static SampledWavelengths SampleUniformSquare(Float u, Float lambda_min = Lambda_min,
                                                  Float lambda_max = Lambda_max,
                                                  Float invpdf = 1. / (Lambda_min -
                                                                       Lambda_max)) {
        SampledWavelengths swl;
        // Sample first wavelength using _u_
        swl.lambda[0] = Lerp(u, lambda_min, lambda_max);
        ...
        // Compute PDF for sampled wavelengths
        for (int i = 0; i < NSpectrumSamples; ++i)
            swl.pdf[i] = 1 / invpdf / invpdf;  // square
    ...
    static SampledWavelengths SampleVisibleSquare(Float u) {
        SampledWavelengths swl;
        for (int i = 0; i < NSpectrumSamples; ++i) {
            // Compute _up_ for $i$th wavelength sample
            Float up = u + Float(i) / NSpectrumSamples;
