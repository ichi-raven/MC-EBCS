...
// -------------------------------------TEST
std::string TestSampler::ToString() const {
    return StringPrintf("[ TestSampler samplesPerPixel: %d seed: %d rng: %s ]",
                        samplesPerPixel, seed, rng);
}

Sampler TestSampler::Clone(Allocator alloc) {
    return alloc.new_object<TestSampler>(*this);
}

TestSampler *TestSampler::Create(const ParameterDictionary &parameters,
                                               const FileLoc *loc, Allocator alloc) {
    int ns = parameters.GetOneInt("pixelsamples", 4);
    if (Options->pixelSamples)
        ns = *Options->pixelSamples;
    int seed = parameters.GetOneInt("seed", Options->seed);
    return alloc.new_object<TestSampler>(ns, seed);
}
...
Sampler Sampler::Create(const std::string &name, const ParameterDictionary &parameters,
    ...
    // Create remainder of _Sampler_ types
        ...
        sampler = TestSampler::Create(parameters, loc, alloc);
    else
