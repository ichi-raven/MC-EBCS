...
std::unique_ptr<IntegratorMS> BasicScene::CreateIntegratorMS(
    std::vector<Camera> cameras, std::vector<Sampler> samplers,
    std::vector<Primitive> accels, std::vector<std::vector<Light>> allLights,
    const EAdaptOption &eAdaptOptions) const {
    return IntegratorMS::Create(integrator.name, integrator.parameters, cameras, samplers,
                                accels, allLights, filmColorSpace, &integrator.loc,
                                eAdaptOptions);
   }
