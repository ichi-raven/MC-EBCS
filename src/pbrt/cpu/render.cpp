...
void RenderCPU(BuiltScene &builtScene) {
    //Allocator alloc;
    ...
    std::unique_ptr<Integrator> integrator(builtScene.pParsedScene->CreateIntegrator(
        builtScene.camera, builtScene.sampler, builtScene.accel, builtScene.lights));
    LOG_VERBOSE("Finished creating integrator");

    // Helpful warnings
    {
        auto &parsedScene = *builtScene.pParsedScene;
        auto &lights = builtScene.lights;
        auto &media = builtScene.media;
        auto &film = builtScene.film;
        auto &materials = builtScene.materials;
        auto &namedMaterials = builtScene.namedMaterials;
        ...
        for (const auto &namedMtl : namedMaterials)
            haveSubsurface |=
                namedMtl.second && namedMtl.second.HasSubsurfaceScattering();
    ...
    if (Options->pixelMaterial) {
        auto &namedMaterials = builtScene.namedMaterials;
        auto &camera = builtScene.camera;
        auto &accel = builtScene.accel;
...
void RenderMultipleScenesCPU(std::vector<BuiltScene> &scenes, const EAdaptOption& eAdaptOptions) {
    // create integrator for multiple scenes
   
    // create camera, sampler, AS, lights vector
    std::vector<Camera> cameras;
    std::vector<Sampler> samplers;
    std::vector<Primitive> accels;
    std::vector<std::vector<Light>> allLights;

    {
        const auto frameNum = scenes.size();

        cameras.resize(frameNum);
        samplers.resize(frameNum);
        accels.resize(frameNum);
        allLights.resize(frameNum);

        for (int i = 0; i < frameNum; ++i) {
            cameras[i] = scenes[i].camera;
            samplers[i] = scenes[i].sampler;
            accels[i] = scenes[i].accel;
            allLights[i] = scenes[i].lights;
        } 
    }
    
    // Integrator
    
    LOG_VERBOSE("Starting to create multiple scene integrators");
    std::unique_ptr<IntegratorMS> integratorMS(
        scenes.front().pParsedScene->CreateIntegratorMS(cameras, samplers, accels,
                                                        allLights, eAdaptOptions));
    LOG_VERBOSE("Finished creating multiple scene integrators");

    // Helpful warnings
    for (const auto& scene : scenes)
    {
        auto &parsedScene = *scene.pParsedScene;
        auto &lights = scene.lights;
        auto &media = scene.media;
        auto &film = scene.film;
        auto &materials = scene.materials;
        auto &namedMaterials = scene.namedMaterials;
