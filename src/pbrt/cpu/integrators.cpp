...
void ImageTileIntegrator::Render() {
    ...
    while (waveStart < spp) {
        ...
        ParallelFor2D(pixelBounds, [&](Bounds2i tileBounds) {
            ...
            progress.Update(static_cast<std::int64_t>(waveEnd - waveStart) *
                            tileBounds.Area());
        });
...
inline double toGray(const RGB &rgb) {
    return rgb.g;
    // return rgb.r * 0.299 + rgb.g * 0.587 + rgb.b * 0.114;
}

// lin-log for event
inline double linLog(double x) {

    const double threshold = 20.0;  // DN20?

    x = std::max(x * 255.0, 1.0e-8);

    const auto f = (1. / threshold) * std::log(threshold);

    auto y = x <= threshold ? x * f : std::log(x);

    const auto rounding = 1e8;
    y = std::round(y * rounding) / rounding;

    return y;
};

// for multiple scenes
void ImageTileIntegratorMS::Render() {
    

    constexpr uint32_t kLowerLimitSample = 256;
    constexpr uint32_t kSampleStep = 64;

    // output parameter
    constexpr int kPaddingNum = 3;

    assert(!integrators.empty() || !"invalid frame num! (empty)");

    // Handle debugStart, if set
    if (!Options->debugStart.empty()) {
        ErrorExit(nullptr, "cannot start debug mode!");
        return;
    }

    // Connect to display server disabled
    if (!Options->displayServer.empty()) {
        ErrorExit(nullptr, "cannot use display server!");
    }

    const int sppBudget = samplerPrototypes.front().SamplesPerPixel();

    std::stringstream thStr;
    thStr << std::fixed << std::setprecision(2) << kThreshold;
    std::stringstream ciStr;
    ciStr << std::fixed << std::setprecision(1) << kAlpha * 100.;

    std::string baseFileName = integrators.front()->GetFilm().GetFilename();

    const std::string outputDirName =
        std::string(kIsUniform ? "uniform" : "ours") +
        std::to_string(sppBudget) + "spp_tau" + thStr.str() + "_" + ciStr.str() + "%" +
        baseFileName.substr(0, baseFileName.find_last_of('.'));
    ...
    const std::string eventDir = outputDirName + "/events/";
    const std::string eventOutputDir = outputDirName + "/eventsOutput/";
    const std::string normalRenderedDir = outputDirName + "/normalRendered/";
    const std::string varianceDir = outputDirName + "/variance/";
    const std::string sampleMapDir = outputDirName + "/samplemap/";

    std::filesystem::create_directory(eventDir);
    std::filesystem::create_directory(eventOutputDir);
    std::filesystem::create_directory(normalRenderedDir);
    std::filesystem::create_directory(varianceDir);
    std::filesystem::create_directory(sampleMapDir);
    ...
    std::vector<std::shared_ptr<ThreadLocal<Sampler>>> samplersVec;
    samplersVec.resize(frameNum);

    for (int i = 0; i < frameNum; ++i) {
        samplersVec[i] = std::make_shared<ThreadLocal<Sampler>>(
            [this, i]() { return samplerPrototypes[i].Clone(); });
    }

    const Bounds2i pixelBounds = cameras.front().GetFilm().PixelBounds();

    for (const auto &camera : cameras) {
        assert(pixelBounds == camera.GetFilm().PixelBounds() ||
               !"invalid scenes (different film size!");
    }

    Image events(PixelFormat::Float, cameras.front().GetFilm().FullResolution(),
                 {"event", "frame", "sampleNum", "logL", " p ", " t "});
    Image eventsOutput(PixelFormat::Float, cameras.front().GetFilm().FullResolution(),
                       {"R", "G", "B"});
    // clear
    ParallelFor2D(pixelBounds, [&](Bounds2i tileBounds) {
        for (Point2i pPixel : tileBounds) {
            events.SetChannel(pPixel, 0, 0.5);
            events.SetChannel(pPixel, 1, 0);

            eventsOutput.SetChannel(pPixel, 0, 0.5);
            eventsOutput.SetChannel(pPixel, 1, 0.5);
            eventsOutput.SetChannel(pPixel, 2, 0.5);
        }
    });

    Image sampleMap = Image(PixelFormat::Float,
                            cameras.front().GetFilm().FullResolution(), {"R", "G", "B"});

    // variance
    Image varImage(PixelFormat::Float, cameras.front().GetFilm().FullResolution(),
                   {"R", "G", "B"});

    const auto toGray = [](const RGB rgb) -> float {
        return rgb.r * 0.299 + rgb.g * 0.587 + rgb.b * 0.114;
    };

    const auto checkEvent = [&](const Point2i pPixel, const int frame) -> float {
        const auto eLogGray =
            toGray(integrators[frame]->GetLogGrayFilm().GetPixelRGB(pPixel));
        const auto eventFrame = static_cast<int>(events.GetChannel(pPixel, 1));

        const auto source =
            toGray(integrators[eventFrame]->GetLogGrayFilm().GetPixelRGB(pPixel));

        if (std::abs(eLogGray - source) >= kThreshold) {
            if (eLogGray > source) {
                return 1.0;
            } else {
                return 0.0;
    ...
    // sample lambda func
    const auto tTest = [&](const Point2i pPixel, const int frame,
                           const int n) -> std::tuple<double, double, bool> {

        const auto eLogGray =
            toGray(integrators[frame]->GetLogGrayFilm().GetPixelRGB(pPixel));

        const auto eSqLogGray =
            toGray(integrators[frame]->GetSquaredFilm().GetPixelRGB(pPixel));
        ...
        const auto sourceSq =
            toGray(integrators[eventFrame]->GetSquaredFilm().GetPixelRGB(pPixel));

        const auto diff = std::abs(source - eLogGray);
        const float sourceSampleNum = sampleMap.GetChannel(pPixel, 0) * sppBudget;

        auto var = (eSqLogGray - (eLogGray * eLogGray)) / static_cast<double>(n - 1.);
        auto varSource =
            (sourceSq - (source * source)) / static_cast<double>(sourceSampleNum - 1.);


        if (var < 0.) {
            fprintf(stderr, "\nminus var!(%lf) at (%d, %d)\n", var, pPixel.x, pPixel.y);
        }
        if (varSource < 0.) {
            fprintf(stderr, "\nminus varSource!(%lf) at (%d, %d)\n", varSource, pPixel.x,
                    pPixel.y);
        }

        var = std::max(0.01, var);
        varSource = std::max(0.01, varSource);

        // calc t
        const double numer = diff - kThreshold;
        const auto sd = std::sqrt(var + varSource);
        const double t_stat = numer / (sd / std::sqrt(static_cast<double>(n)));

        // calc p-value
        boost::math::students_t dist(std::max(1.0, sourceSampleNum + n - 2.0));
        return {boost::math::cdf(dist, t_stat), t_stat, source < eLogGray};
    };

    const auto sample = [&](const int sampleStart, const int sampleEnd, const int frame,
                            ProgressReporter &progress) {
        auto &integrator = integrators[frame];
        Sampler &sampler = samplersVec[frame]->Get();
        const int half = (sampleEnd - sampleStart) / 2;
        // Render current wave's image tiles in parallel
        ParallelFor2D(pixelBounds, [&](Bounds2i tileBounds) {
            // Render image tile given by _tileBounds_
            ...
            for (Point2i pPixel : tileBounds) {
                ...
                // Render samples in pixel _pPixel_
                for (int sampleIndex = sampleStart; sampleIndex < sampleEnd;
                     ++sampleIndex) {
                    const bool useAdditionalFilm = sampleIndex >= half;

                    threadSampleIndex = sampleIndex;
                    sampler.StartPixelSample(pPixel, sampleIndex);
                    integrator->EvaluatePixelSample(pPixel, sampleIndex, sampler,
                                                    scratchBuffer, useAdditionalFilm,
                                                    nullptr, kThreshold);

                    scratchBuffer.Reset();
                }

                events.SetChannel(pPixel, 2, static_cast<float>(sampleEnd - sampleStart));

                events.SetChannel(
                    pPixel, 3,
                    integrators[frame]->GetLogGrayFilm().GetPixelRGB(pPixel).g);
                events.SetChannel(pPixel, 4, 0.5);
                events.SetChannel(pPixel, 5, 0.);

                sampleMap.SetChannel(pPixel, 0, 1.0);
                sampleMap.SetChannel(pPixel, 1, 1.0);
                sampleMap.SetChannel(pPixel, 2, 1.0);
            ...
            PBRT_DBG("Finished image tile (%d,%d)-(%d,%d)\n", tileBounds.pMin.x,
                     tileBounds.pMin.y, tileBounds.pMax.x, tileBounds.pMax.y);
            progress.Update((sampleEnd - sampleStart) * tileBounds.Area());
        });
    };

    const auto sampleUntilEvent = [&](const int waveSize, const int limitSpp,
                                      const int frame, ProgressReporter &progress) {
        auto &integrator = integrators[frame];
        Sampler &sampler = samplersVec[frame]->Get();
        if (waveSize <= 2) {
            fprintf(stderr, "\n\ninvalid wave size(%d)!!!!\n", waveSize);
        ...
        ParallelFor2D(pixelBounds, [&](Bounds2i tileBounds) {
            ...
            for (Point2i pPixel : tileBounds) {
                ...
                int n = 0;
                // if change adding sample, change here
                bool endSampling = false;

                while (!endSampling) {
                    const int sampleSpp =
                        n + waveSize > static_cast<int>(kLowerLimitSample)
                            ? std::min(limitSpp - n, waveSize)
                            : kLowerLimitSample;

                    // Render samples in pixel _pPixel_
                    for (int sampleIndex = n; sampleIndex < n + sampleSpp;
                         ++sampleIndex) {
                        const bool useAdditionalFilm = sampleIndex - n >= (sampleSpp / 2);
                    ...
                    // t-test
                    const auto [p, t, isPositive] = tTest(pPixel, frame, n);

                    // for double sided
                     const bool rejected =
                         !kIsUniform && (std::abs(p - 0.5) >= kAlpha
                         / 2.0);

                    //  end if rejected or exceeded limit spp
                    endSampling = rejected || n >= limitSpp;

                    if (endSampling) {
                        if (rejected) {
                            if (p >= 0.5) {
                                // event
                                events.SetChannel(pPixel, 0, isPositive ? 1.0 : 0.0);
                                events.SetChannel(pPixel, 1, frame);
                            } else {
                                // none
                                events.SetChannel(pPixel, 0, 0.5);
                            }
                        } else {  // normal event check (not rejected)
                            const auto val = checkEvent(pPixel, frame);
                            events.SetChannel(pPixel, 0, val);
                            if (val != 0.5) {
                                events.SetChannel(pPixel, 1, frame);
                            }
                        }

                        events.SetChannel(pPixel, 2, static_cast<float>(n));

                        events.SetChannel(
                            pPixel, 3,
                            toGray(integrators[frame]->GetLogGrayFilm().GetPixelRGB(
                                pPixel)));

                        events.SetChannel(pPixel, 4, p);
                        events.SetChannel(pPixel, 5, t);
                    }
                }

                const double sampledRate = 1. * n / limitSpp;
                sampleMap.SetChannel(pPixel, 0, sampledRate);
                sampleMap.SetChannel(pPixel, 1, sampledRate);
                sampleMap.SetChannel(pPixel, 2, sampledRate);
    ...
    const auto updateVariance = [&](const int frame) {
        auto &filmSq = integrators[frame]->GetSquaredFilm();
        // update var image
        ParallelFor2D(filmSq.PixelBounds(), [&](const Bounds2i tileBounds) {
            for (const auto pPixel : tileBounds) {
                const auto eLogGray =
                    integrators[frame]->GetLogGrayFilm().GetPixelNativeRGB(pPixel).g;
                const auto eSqLogGray = filmSq.GetPixelNativeRGB(pPixel).g;
                if (eSqLogGray < 0.) {
                    fprintf(stderr, "invalid SqLogGray(%lf) at (%d, %d)\n", eSqLogGray,
                            pPixel.x, pPixel.y);
                }

                auto var = (eSqLogGray - (eLogGray * eLogGray));
                if (var < -0.01) {
                    fprintf(stderr, "\nminus var!(%lf - %lf) at (%d, %d)\n", eSqLogGray,
                            (eLogGray * eLogGray), pPixel.x, pPixel.y);
                    var = 0.01;
                }

                varImage.SetChannel(pPixel, 0, var);
                varImage.SetChannel(pPixel, 1, eLogGray * eLogGray);
                varImage.SetChannel(pPixel, 2, eSqLogGray);
            }
        });
    };

    const auto finalRes = [&](const int frame) -> Image {
        ImageMetadata metadata;

        Image result = integrators[frame]->GetFilm().GetImage(&metadata);

        ParallelFor2D(pixelBounds, [&](Bounds2i tileBounds) {
            for (Point2i pPixel : tileBounds) {
                for (int i = 0; i < 3; ++i) {
                    result.SetChannel(pPixel, i,
                                      integrators[frame]->GetPixelRGBAvg(pPixel)[i]);
                }
            }
        });
    ...
    const auto outputResult = [&](const int frame, const double elapsedSeconds) {
        ImageMetadata metadata;

        metadata.renderTimeSeconds = elapsedSeconds;
        metadata.samplesPerPixel = sppBudget;
        Image result = finalRes(frame);

        const std::string frameStr = std::to_string(frame);
        const std::string paddedFrame =
            std::string(kPaddingNum - frameStr.length(), '0') + frameStr;

        {  // final result

            std::string filename = integrators[frame]->GetFilm().GetFilename();
            result.Write(normalRenderedDir + filename);

            updateVariance(frame);
            filename = "variance_" + paddedFrame + ".exr";
            varImage.Write(varianceDir + filename);
        }

        {  // output events
            std::string filename = "events_" + paddedFrame + ".exr";
            events.Write(eventDir + filename);

            ParallelFor2D(pixelBounds, [&](Bounds2i tileBounds) {
                for (Point2i pPixel : tileBounds) {
                    for (int color = 0; color < 3; ++color) {
                        eventsOutput.SetChannel(pPixel, color,
                                                events.GetChannel(pPixel, 0));
                    }
                }
            });

            filename = "eventsOutput_" + paddedFrame + ".exr";
            eventsOutput.Write(eventOutputDir + filename);
        }

        {  // output sample map
            std::string filename = "samplemap_" + paddedFrame + ".exr";
            sampleMap.Write(sampleMapDir + filename);
        }
    };

    // main logic---------------------------------------------------
    double initialSampleTime = 0.;
    double adaptiveSampleTime = 0.;

    {  // initial sampling
        ProgressReporter progress(int64_t(sppBudget) * pixelBounds.Area(),
                                  "Initial frame sampling", Options->quiet);

        sample(0, sppBudget, 0, progress);

        // maybe filtering would help ?
        initialSampleTime = progress.ElapsedSeconds();
        outputResult(0, initialSampleTime);
        progress.Done();
    }  // end of initial sampling

    {  // adaptive sampling
        ProgressReporter progress(pixelBounds.Area() * frameNum, "Adaptive sampling",
                                  Options->quiet);
        ImageMetadata filmMetadata;

        for (int frame = 1; frame < frameNum; ++frame) {
            sampleUntilEvent(kSampleStep, sppBudget, frame, progress);

            // fprintf(stderr, "\nframe %d end\n", frame);
            outputResult(frame, progress.ElapsedSeconds());
        }

        adaptiveSampleTime = progress.ElapsedSeconds();
        progress.Done();
    }  // end if adaptive sampling

    LOG_VERBOSE("Rendering finished");
    fprintf(stderr, "\ntotal elapsed time : %lf[sec]\n",
            initialSampleTime + adaptiveSampleTime);
    {
        FILE *fp = fopen((outputDirName + "/time.txt").c_str(), "w");
        fprintf(fp, "total elapsed time : %lf[sec]\n",
                initialSampleTime + adaptiveSampleTime);
        fprintf(fp, "\tinitial sampling time : %lf[sec]\n", initialSampleTime);
        fprintf(fp, "\tadaptive sampling time : %lf[sec]\n", adaptiveSampleTime);
        fprintf(fp, "\tadaptive sampling per frame average : %lf[sec]\n",
                adaptiveSampleTime / frameNum);
        fclose(fp);
    }
}
...
void RayIntegrator::EvaluatePixelSample(Point2i pPixel, int sampleIndex, Sampler sampler,
                                        ...
                                        const bool useAdditionalFilm, RGB *pRGBVal,
    ...
    //   SampledWavelengths::SampleConstant(630.0F, 532.0F, 467.0F);
    // SampledWavelengths lambda =
    // SampledWavelengths::SampleConstant(532.0F, 532.0F, 532.0F);
    ...
    if (cameraRay) {
        ...
        if (cameraRay)
            PBRT_DBG("%s\n",
                     StringPrintf("Camera sample: %s -> ray %s "
                                  "-> L = %s, visibleSurface %s",
    ...
    // HACK:
    if (pRGBVal) {
        *pRGBVal = camera.GetFilm().GetPixelSensor()->ToSensorRGB(L, lambda);
    }

    // Add camera ray's contribution to image
    if (useAdditionalFilm) {
        additionalFilm.AddSample(pPixel, L, lambda, &visibleSurface, 1.0);
    } else {
        camera.GetFilm().AddSample(pPixel, L, lambda, &visibleSurface, 1.0);
    }

    logGrayFilm.AddSampleLogGray(pPixel, L, lambda, &visibleSurface, 1.0, threshold);
    squareMeanFilm.AddSampleSquare(pPixel, LSq, lambda, &visibleSurface, 1.0, threshold);
}
...
SampledSpectrum SimplePathIntegrator::Li(RayDifferential ray, SampledWavelengths &lambda,
                                         ...
                                         VisibleSurface *, SampledSpectrum *) const {
    ...
    while (beta) {
        ...
        if (sampleLights) {
            ...
            if (sampledLight) {
                // Sample point on _sampledLight_ to estimate direct
                // illumination
                Point2f uLight = sampler.Get2D();
                pstd::optional<LightLiSample> ls =
                    sampledLight->light.SampleLi(isect, uLight, lambda);
                if (ls && ls->L && ls->pdf > 0) {
                    // Evaluate BSDF for light and possibly add scattered
                    // radiance
                    Vector3f wi = ls->wi;
        ...
        if (sampleBSDF) {
            ...
            // Uniformly sample sphere or hemisphere to get new path
            // direction
            Float pdf;
            Vector3f wi;
    ...
    if (les->intr) {
        ...
        if (cs && cs->pdf != 0) {
            if (Float pdf = light.PDF_Li(cs->pLens, -cs->wi); pdf > 0) {
                // Add light's emitted radiance if nonzero and light is
                // visible
                SampledSpectrum Le =
                    light.L(les->intr->p(), les->intr->n, les->intr->uv, cs->wi, lambda);
                if (Le && Unoccluded(cs->pRef, cs->pLens)) {
                    // Compute visible light's path contribution and add
                    // to film
...
SampledSpectrum PathIntegrator::Li(RayDifferential ray, SampledWavelengths &lambda,
                                   Sampler sampler, ScratchBuffer &scratchBuffer,
                                   VisibleSurface *visibleSurf, SampledSpectrum *) const {
...
// PathIntegratorMod Method Definitions
PathIntegratorMod::PathIntegratorMod(int maxDepth, Camera camera, Sampler sampler,
                                     Primitive aggregate, std::vector<Light> lights,
...
SampledSpectrum PathIntegratorMod::Li(RayDifferential ray, SampledWavelengths &lambda,
                                      Sampler sampler, ScratchBuffer &scratchBuffer,
                                      VisibleSurface *visibleSurf,
                                      SampledSpectrum *pLiSq) const {
    // Declare local variables for _PathIntegrator::Li()_
    SampledSpectrum L(0.f), beta(1.f), LSq(0.f);
    int depth = 0;
    Float p_b, etaScale = 1;
    bool specularBounce = false, anyNonSpecularBounces = false;
    LightSampleContext prevIntrCtx;
    ...
    while (true) {
        ...
        if (!si) {
            // Incorporate emission from infinite lights for escaped ray
            for (const auto &light : infiniteLights) {
                SampledSpectrum Le = light.Le(ray, lambda).each(linLog);  // linLog
                if (depth == 0 || specularBounce) {
                    L += beta * Le;
                    LSq += (beta * beta * Le * Le);  // LSq
                } else {
                    // Compute MIS weight for infinite light
                    Float p_l = lightSampler.PMF(prevIntrCtx, light) *
                                light.PDF_Li(prevIntrCtx, ray.d, true);
        ...
        if (IsNonSpecular(bsdf.Flags())) {
            ++totalPaths;
            auto [Ld, w_l] = SampleLd(isect, &bsdf, lambda, sampler);
            if (!Ld)
                ++zeroRadiancePaths;
            Ld.each(linLog);  // linLog
            L += w_l * beta * Ld;
            LSq += w_l * (beta * beta * Ld * Ld);  // LSq
        }
...
std::pair<SampledSpectrum, Float> PathIntegratorMod::SampleLd(
    const SurfaceInteraction &intr, const BSDF *bsdf, SampledWavelengths &lambda,
    Sampler sampler) const {
    // Initialize _LightSampleContext_ for light sampling
    LightSampleContext ctx(intr);
    // Try to nudge the light sampling position to correct side of the surface
    ...
    Float p_l = sampledLight->p * ls->pdf;
    if (IsDeltaLight(light.Type()))
        return {ls->L * f / p_l, 1.0};
    else {
        Float p_b = bsdf->PDF(wo, wi);
        Float w_l = PowerHeuristic(1, p_l, 1, p_b);
        return {ls->L * f / p_l, w_l};
    }
}

std::string PathIntegratorMod::ToString() const {
    return StringPrintf("[ PathIntegrator maxDepth: %d lightSampler: %s regularize: %s ]",
                        maxDepth, lightSampler, regularize);
}

std::unique_ptr<PathIntegratorMod> PathIntegratorMod::Create(
    const ParameterDictionary &parameters, Camera camera, Sampler sampler,
    Primitive aggregate, std::vector<Light> lights, const FileLoc *loc) {
    int maxDepth = parameters.GetOneInt("maxdepth", 5);
    std::string lightStrategy = parameters.GetOneString("lightsampler", "bvh");
    bool regularize = parameters.GetOneBool("regularize", false);
    return std::make_unique<PathIntegratorMod>(maxDepth, camera, sampler, aggregate,
...
SimpleVolPathIntegrator::SimpleVolPathIntegrator(int maxDepth, Camera camera,
    ...
    for (Light light : lights) {
        if (IsDeltaLight(light.Type()))
            ErrorExit("SimpleVolPathIntegrator only supports area and "
                      "infinite light "
                      "sources");
    }
}
    ...
    while (true) {
        ...
        if (ray.medium) {
            ...
            SampleT_maj(ray, tMax, u, rng, lambda,
                        [&](Point3f p, MediumProperties mp, SampledSpectrum sigma_maj,
                            ...
                            // Randomly sample medium scattering event for delta
                            // tracking
                            ...
                            if (mode == 0) {
                                ...
                                // Handle regular scattering event for medium
                                // sample Stop path sampling if maximum depth has
                                // been reached
                                if (depth++ >= maxDepth) {
                                    terminated = true;
                                    return false;
                                ...
                                // Sample phase function for medium scattering
                                // event
        ...
        if (!bsdf)
            ...
            if (bsdf.Sample_f(-ray.d, uc, u))
                ErrorExit("SimpleVolPathIntegrator doesn't support surface "
                          "scattering.");
            else
                break;
        }
...
SampledSpectrum VolPathIntegrator::Li(RayDifferential ray, SampledWavelengths &lambda,
    ...
    while (true) {
            ...
            SampledSpectrum T_maj = SampleT_maj(
                ...
                [&](Point3f p, MediumProperties mp, SampledSpectrum sigma_maj,
                    ...
                    if (depth < maxDepth && mp.Le) {
                        ...
                        // Compute rescaled path probability for
                        // absorption at path vertex
                    ...
                    if (mode == 0) {
                        ...
                        // Update _beta_ and _r_u_ for real-scattering
                        // event
                        ...
                        if (beta && r_u) {
                            ...
                            // Sample new direction at real-scattering
                            // event
                            Point2f u = sampler.Get2D();
                            pstd::optional<PhaseFunctionSample> ps =
                            ...
                            if (!ps || ps->pdf == 0)
                                ...
                                // Update ray path state for indirect
                                // volume scattering
                                beta *= ps->p / ps->pdf;
                                r_l = r_u / ps->pdf;
            ...
            for (const auto &light : infiniteLights) {
                if (SampledSpectrum Le = light.Le(ray, lambda); Le) {
                    if (depth == 0 || specularBounce)
                        ...
                        // Add infinite light contribution using both PDFs
                        // with MIS
        ...
        if (bssrdf && bs->IsTransmission()) {
            ...
            // Account for attenuated direct illumination subsurface
            // scattering
            ...
            SampledSpectrum T_maj =
                SampleT_maj(lightRay, tMax, u, rng, lambda,
                            [&](Point3f p, MediumProperties mp, SampledSpectrum sigma_maj,
                                ...
                                // Update _T_ray_ and PDFs using ratio-tracking
                                // estimator
                                SampledSpectrum sigma_n =
                                    ClampZero(sigma_maj - mp.sigma_a - mp.sigma_s);
...
struct Vertex {
    ...
    Float PDFLight(const Integrator &integrator, const Vertex &v) const {
        ...
        if (IsInfiniteLight()) {
            ...
            if (type == VertexType::Light)
                CHECK(ei.light.Is<DiffuseAreaLight>());  // since that's
                                                         // all we've got
                                                         // currently...
...
int GenerateLightSubpath(const Integrator &integrator, SampledWavelengths &lambda,
    ...
    PBRT_DBG("%s\n", StringPrintf("Starting light subpath. Ray: %s, Le %s, beta "
                                  "%s, pdfPos %f, pdfDir %f",
            ...
            SampledSpectrum T_maj = SampleT_maj(
                ...
                [&](Point3f p, MediumProperties mp, SampledSpectrum sigma_maj,
                    ...
                    if (mode == 0) {
                        ...
                        // Record medium interaction in _path_ and compute
                        // forward density
                        ...
                        // Sample direction and compute reverse density at
                        // preceding vertex
                        pstd::optional<PhaseFunctionSample> ps =
                        ...
                        // Update path state and previous path vertex
                        // after medium scattering
                        pdfFwd = ps->pdf;
                        beta *= ps->p / ps->pdf;
...
SampledSpectrum BDPTIntegrator::Li(RayDifferential ray, SampledWavelengths &lambda,
    ...
    for (int t = 1; t <= nCamera; ++t) {
        for (int s = 0; s <= nLight; ++s) {
            ...
            PBRT_DBG("%s\n", StringPrintf("Connect bdpt s: %d, t: %d, "
                                          "Lpath: %s, misWeight: %f\n",
                                          s, t, Lpath, misWeight)
                                 .c_str());
            if (Lpath && (visualizeStrategies || visualizeWeights)) {
                ...
                if (pFilmNew)
                    ...
                    // need to go back to the camera and ask for them;
                    // here we take a point a little bit along the camera
                    // ray and ask the camera to reproject that for us.
                    //
                    // Double unfortunately, this doesn't quite work for
                    // scenes where the camera has a finite aperture,
                    // since we don't have the CameraSample either so just
                    // have to pass (0.5,0.5) in for the lens sample...
...
SampledSpectrum ConnectBDPT(const Integrator &integrator, SampledWavelengths &lambda,
    ...
    if (s == 0) {
        ...
        if (qs.IsConnectible()) {
            ...
            if (cs) {
                *pRaster = cs->pRaster;
                // Initialize dynamically sampled vertex and _L_ for $t=1$
                // case
        ...
        if (pt.IsConnectible()) {
            ...
            if (sampledLight) {
                ...
                if (pt.IsOnSurface()) {
                    ...
                    // Try to nudge the light sampling position to correct
                    // side of the surface
                ...
                if (lightWeight && lightWeight->L && lightWeight->pdf > 0) {
                    ...
                    // Only check visibility if the path would carry
                    // radiance.
                    if (L)
        ...
        if (qs.IsConnectible() && pt.IsConnectible()) {
            ...
            PBRT_DBG("%s\n",
                     StringPrintf("General connect s: %d, t: %d, qs: %s, "
                                  "pt: %s, qs.f(pt): %s, "
...
void MLTIntegrator::Render() {
    ...
    if (!Options->debugStart.empty()) {
        ...
        if (c.empty())
            ErrorExit("Didn't find comma-separated values after "
                      "--debugstart: %s",
                      Options->debugStart);
    ...
    ParallelFor(0, nBootstrap, [&](int64_t start, int64_t end) {
        ...
        for (int64_t i = start; i < end; ++i) {
            ...
            for (int depth = 0; depth <= maxDepth; ++depth) {
                ...
                threadDepth = depth;
                // Evaluate path radiance using _sampler_ and update
                // _bootstrapWeights_
                Point2f pRaster;
                SampledWavelengths lambda;
        ...
        ParallelFor2D(pixelBounds, [&](Bounds2i tileBounds) {
            ...
            for (Point2i pPixel : tileBounds) {
                ...
                while (true) {
                    ...
                    if (!si) {
                        SampledSpectrum L(0.f);
                        // Incorporate emission from infinite lights for escaped
                        // ray
                        for (const auto &light : infiniteLights) {
                            SampledSpectrum Le = light.Le(ray, lambda);
                            if (depth == 0 || specularBounce)
                    ...
                    ++totalBSDFs;
                    // Add emission from directly visible emissive surfaces to
                    // _pixel.Ld_
                    Vector3f wo = -ray.d;
                    SampledSpectrum L(0.f);
                    // Incorporate emission from surface hit by ray
                    ...
                    // Terminate path at maximum depth or if visible point has
                    // been set
        ...
        ParallelFor2D(pixelBounds, [&](Bounds2i tileBounds) {
            ...
            for (Point2i pPixel : tileBounds) {
                ...
                if (pixel.vp.beta) {
                    ...
                    // Find grid cell bounds for pixel's visible point, _pMin_ and
                    // _pMax_
    ...
    for (int sampleIndex = 0; sampleIndex < nSamples; ++sampleIndex) {
        ...
        if (isStratified) {
            ...
            ParallelFor2D(pixelBounds, [&](Point2i pPixel) {
                ...
                // Need nTakenSamples factor to cancel out division in
                // sumSE computation.
...
std::unique_ptr<Integrator> Integrator::Create(
    ...
    if (name == "path")
        // integrator =
        // PathIntegrator::Create(parameters, camera, sampler, aggregate, lights,
        // loc);
        integrator = PathIntegratorMod::Create(parameters, camera, sampler, aggregate,
                                               lights, loc);
...
std::unique_ptr<IntegratorMS> IntegratorMS::Create(
    ...
    if (name == "lightpath" || name == "function" || name == "mlt" || name == "sppm") {
        ErrorExit(loc, "integrator %s is not supported temporal adaptive sampling!",
                  name);
        exit(-1);
    }

    return std::make_unique<ImageTileIntegratorMS>(name, parameters, cameras, samplers,
                                                   aggregates, lights, colorSpace, loc, options);
}
}  // namespace pbrt
