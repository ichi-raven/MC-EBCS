...
// TestSampler Definition (FOR TEST)
class TestSampler {
  public:
    // TestSampler Public Methods
    TestSampler(int samplesPerPixel, int seed = 0)
        : samplesPerPixel(samplesPerPixel), seed(seed) {}

    static TestSampler *Create(const ParameterDictionary &parameters, const FileLoc *loc,
                               Allocator alloc);
    PBRT_CPU_GPU
    static constexpr const char *Name() { return "TestSampler"; }
    ...
    Point2f Get2D() { return {rng.Uniform<Float>(), rng.Uniform<Float>()}; }
    PBRT_CPU_GPU
    Point2f GetPixel2D() { return {0.5, 0.5}; }

    Sampler Clone(Allocator alloc);
    std::string ToString() const;

  private:
    // TestSampler Private Members
    int samplesPerPixel, seed;
    RNG rng;
};
// --------------------------------------------END TEST
