...
PBRT_CPU_GPU
void GBufferFilm::AddSampleLogGray(Point2i pFilm, SampledSpectrum L,
                                   const SampledWavelengths &lambda,
                                   const VisibleSurface *visibleSurface, Float weight,
                                   Float threshold) {
    RGB rgb = sensor->ToSensorRGB(L, lambda);
    //const auto gray = rgb.r * 0.299 + rgb.g * 0.587 + rgb.b * 0.114;
    //rgb = RGB(gray, gray, gray);
     ...
     //    rgb *= maxComponentValue / m;
     //const auto gray = rgb.r * 0.299 + rgb.g * 0.587 + rgb.b * 0.114;
     //const auto logGray = linLog(gray);
     //rgb = RGB(logGray, logGray, logGray);

    //const auto avg = sampled.g;  // SafeDiv(L, lambda.PDF()).Average();
    // RGB rgb(avg, avg, avg);
    //RGB rgb = sampled;
...
// GBufferFilm Method Definitions
void GBufferFilm::AddSampleSquare(Point2i pFilm, SampledSpectrum LSq,
                                  const SampledWavelengths &lambda,
                                  const VisibleSurface *visibleSurface, Float weight,
                                  Float threshold) {
    if (LSq.MinComponentValue() < 0.) {
        fprintf(stderr, "error! at (%d, %d) : %lf\n\n", pFilm.x, pFilm.y,
                LSq.MinComponentValue());
    }

    // const SampledSpectrum L = LSq.Sqrt();
    
    RGB rgb = sensor->ToSensorRGB(LSq, SampledWavelengths::squarePDF(lambda));
    //const auto gray = rgb.r * 0.299 + rgb.g * 0.587 + rgb.b * 0.114;
    //rgb = RGB(gray, gray, gray);
