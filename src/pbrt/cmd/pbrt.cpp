...
Reformatting options:
  ...
  --threshold                   Threshold of event-based camera.
  --alpha                       confidence interval for hypothesis-testing.
  --uniform                     use baseline method (not ours)
)",
            NSpectrumSamples);
    exit(msg.empty() ? 0 : 1);
}
...
int main(int argc, char *argv[]) {
    ...
    for (auto iter = args.begin(); iter != args.end(); ++iter) {
        ...
        if (ParseArg(&iter, args.end(), "cropwindow", &cropWindow, onError)) {
            ...
            // added
            ParseArg(&iter, args.end(), "threshold", &options.threshold, onError) ||
            ParseArg(&iter, args.end(), "alpha", &options.alpha, onError) ||
            ParseArg(&iter, args.end(), "uniform", &options.isUniform, onError)) {
            // success
    ...
    fprintf(stdout, "uniform : %d\n", options.isUniform);

    std::vector<std::string> filenames;

    std::string directory_path = baseFileName;

    for (const auto &entry : std::filesystem::directory_iterator(directory_path)) {
        if (entry.path().extension() == ".pbrt") {
    ...
    if (format || toPly || options.upgrade) {
        ...
        pstd::vector<BasicScene *> pScenes;
        pstd::vector<ParserTarget *> pBuilders;
        pScenes.reserve(filenames.size());
        pBuilders.reserve(filenames.size());

        for (std::size_t i = 0; i < filenames.size(); ++i) {
            pScenes.emplace_back(new BasicScene());
            pBuilders.emplace_back(new BasicSceneBuilder(pScenes.back()));
        }

        std::vector<BuiltScene> builtScenes;

        ParseFilesIndependent(pBuilders, filenames, builtScenes);

        if (Options->useGPU || Options->wavefront) {
            Warning("Disabled GPU rendering temporary (use CPU mode...)");
        }
        RenderMultipleScenesCPU(
            builtScenes,
            EAdaptOption{options.threshold, options.alpha, options.isUniform});

        for (auto p : pScenes) {
            delete p;
        }
        for (auto p : pBuilders) {
            delete p;
        }
        ...
        fprintf(stdout, "all tasks finished successfully!\n");
    }
    return 0;
}
